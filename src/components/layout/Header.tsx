        ...
        <div className="md:hidden bg-white border-t border-stone-100 px-4 py-4 space-y-1">
            ...
            return (
              <Link key={link.to} to={link.to}
                className={`block px-4 py-3 rounded-xl text-sm font-medium transition-all ${
                  active ? 'bg-amber-50 text-amber-700' : 'text-stone-600 hover:bg-stone-100'
                }`}>
                {link.label}
              </Link>
            );
          ...
          <Link to="/favoritos"
            className={`flex items-center justify-between px-4 py-3 rounded-xl text-sm font-medium transition-all ${
              location.pathname === '/favoritos' ? 'bg-amber-50 text-amber-700' : 'text-stone-600 hover:bg-stone-100'
            }`}>
